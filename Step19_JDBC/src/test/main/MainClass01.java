 ...
 * 
 * Oracle에 연결하기 위해서는 드라이버 클래스가 들어있는 ojdbc6.jar 파일을 사용 할 수 있도록 설정해야 한다.
 */
public class MainClass01 {
	public static void main(String[] args) {
		...
		try {
			...
			pstmt = conn.prepareStatement(sql);//실행할 sql문 준비
			//PreparedStatement 객체를 이용해서 query문을 수행하고 결과를 ResultSet 객체로 받아오기
			rs=pstmt.executeQuery();//결과 데이터를 ResultSet으로 받아온다.
			while(rs.next()) {//ResultSet객체의 next 메소드 boolean타입 리턴
